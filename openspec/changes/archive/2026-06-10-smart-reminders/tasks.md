...
- [x] 1.1 `App/Notifications/ReminderScheduler.swift`：請求授權；依 `TrainingPlan` 非休息日於指定時間排程 local notification；關閉時清除全部。Requirement: Opt-in training reminders; Permission respected。
...
- [x] 2.1 `SettingsView` 加「Reminders」區：開關 + 時間選擇；綁定 `ReminderScheduler`。
